...
function App() {
  ...
  useEffect(() => {
    console.log(
      `M${[0, 4, 8, 2, 6]
        .map(i => [
          polarCoords(pentaAngles[i], center).x,
          polarCoords(pentaAngles[i], center).y,
        ])
        .join(' L')} Z`
    );
  }, []);
    ...
    <div className="App">
      ...
      >
        {quintaAngles.map(a => (
          <g key={a}>
            <line
              className="stroked dim"
              x1={polarCoords(a, r4).x}
              y1={polarCoords(a, r4).y}
              x2={polarCoords(a, r1).x}
              y2={polarCoords(a, r1).y}
            />
            <text
              dominantBaseline="middle"
              textAnchor="middle"
              x={polarCoords(a, r2).x}
              y={polarCoords(a, r2).y}
            >
              {a}
            </text>
          </g>
        ...
        <line
          className="stroked bright"
          x1={center}
          y1={center}
          x2={polarCoords(closestAngle, closestRadius).x}
          y2={polarCoords(closestAngle, closestRadius).y}
        />
      </svg>
    </div>
  );
